...
function getRequest(provider: string): Request {
  return new Request(`http://localhost/api/task-sync/${provider}/sync-status`)
}
...
describe("GET /api/task-sync/[provider]/sync-status (integration)", () => {
  it("authenticates by delegation to getTaskSyncStatus: an unauthenticated call is refused with 401", async () => {
    ...
    const res = await GET(getRequest("linear"), params("linear"))
  ...
  it("returns 404 for an unregistered provider", async () => {
    const { GET } = await import("./route")
    const res = await GET(getRequest("github"), params("github"))

    expect(res.status).toBe(404)
  })

  it("returns idle when the caller has no sync run for that provider", async () => {
    const { GET } = await import("./route")
    const res = await GET(getRequest("linear"), params("linear"))
