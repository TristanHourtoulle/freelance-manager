...
/**
 * Route runs three `task.findMany` calls (pipeline, recently completed and
 * in-progress) through one mock; branch on the where clause status to return
 * each dataset.
 */
function mockTaskFindMany(
  ...
  taskFindMany.mockImplementation((args: TaskFindManyArgs) => {
    if (args?.where?.status === "PENDING_INVOICE") {
