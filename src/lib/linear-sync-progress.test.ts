...
const { prismaMock } = vi.hoisted(() => ({
  prismaMock: {
    taskSyncRun: { create: vi.fn(), update: vi.fn() },
  },
}))
...
import {
  ...
  failSyncRun,
  touchSyncRun,
} from "@/lib/task-sync/progress"

describe("createSyncRun", () => {
  beforeEach(() => {
    vi.clearAllMocks()
    prismaMock.taskSyncRun.create.mockResolvedValue({ id: "run-1" })
  })

  it("opens a RUNNING row for the user and returns its id", async () => {
    const id = await createSyncRun("user-1", "linear", 3)

    expect(id).toBe("run-1")
    expect(prismaMock.taskSyncRun.create).toHaveBeenCalledWith({
      data: {
        userId: "user-1",
        providerId: "linear",
        totalMappings: 3,
...
describe("touchSyncRun", () => {
  ...
  it("writes only the supplied progress fields", async () => {
    ...
    expect(prismaMock.taskSyncRun.update).toHaveBeenCalledWith({
      where: { id: "run-1" },
  ...
  it("swallows and logs a database error instead of rethrowing", async () => {
    ...
    prismaMock.taskSyncRun.update.mockRejectedValue(new Error("db down"))
